...
parser.add_argument('--dataset', choices=['BayArea', 'Hermiston', 'farmland','river'], default='farmland', help='dataset to use')
...
parser.add_argument('--epoches', type=int, default=80, help='epoch number')
...
if args.dataset == 'BayArea':
    ...
    data_t1 = loadmat("./data/Hermiston/hermiston2004.mat")['HypeRvieW']
    data_t2 = loadmat("./data/Hermiston/hermiston2007.mat")['HypeRvieW']
    data_label = loadmat("./data/Hermiston/rdChangesHermiston_5classes.mat")['gt5clasesHermiston']
    uc_position = np.array(np.where(data_label==2)).transpose(1,0)
    c_position = np.array(np.where(data_label==1)).transpose(1,0)
    print((uc_position.shape[0],c_position.shape[0]))

elif args.dataset == 'river':
    data_t1 = loadmat('./data/river/river_before.mat')['river_before']
    data_t2 = loadmat('./data/river/river_after.mat')['river_after']
    data_label = loadmat('./data/river/groundtruth.mat')['lakelabel_v1']
